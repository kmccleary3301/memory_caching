...
| Data processing emits deterministic tokenized shards with source distribution evidence | Pipeline scripts + manifests | `scripts/data/process_data.py`, `artifacts/data_manifest.json` |
| Training pipeline writes actual torch checkpoints with resume metadata and loss traces | Pipeline scripts + checkpoint contract | `scripts/train/train_loop.py`, `docs/CHECKPOINT_CONTRACT.md`, `artifacts/train_manifest.json` |
