...
| Full-step/full-sequence training matrix can be executed with compile+AMP and archived per profile | Full-run checkpoints + telemetry manifests + parity table | `artifacts/checkpoints/pilot_full/`, `artifacts/checkpoints/mid_full/`, `artifacts/checkpoints/target_full/`, `outputs/reports/training_telemetry/`, `outputs/reports/training_parity_table_full.json`, `docs/TRAINING_PARITY_TABLE_FULL.md` |
...
| Paper-scale execution path is codified in a single script with subset-safety checks | Execution script + scaling run plan | `scripts/checks/paper_scale_execution.sh`, `docs/SCALING_GAP_RUN_PLAN.md` |
...
| CI blocks accidental large artifacts/weights from entering git history | Guardrail script + CI workflow step | `scripts/checks/no_large_artifacts.sh`, `.github/workflows/repro_checks.yml` |
